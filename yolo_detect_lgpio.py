...
try:
    ...
    print('WARNING: lgpio not found. Install with: pip install lgpio')
    SERVO_AVAILABLE = False

def set_servo_angle(chip_handle, gpio_pin, angle):
    """Move servo to angle (0-180) using lgpio hardware-timed PWM.
    Sends the signal for a short burst then stops to avoid jitter."""
    # Convert angle to pulse width: 500us (0deg) to 2500us (180deg)
    pulse_us = 500 + (angle / 180.0) * 2000
    freq = 50  # 50Hz for servos
    # duty cycle as fraction of period (20000us)
    duty = pulse_us / 20000.0 * 100.0  # as percentage
    lgpio.tx_pwm(chip_handle, gpio_pin, freq, duty)
    
def stop_servo(chip_handle, gpio_pin):
    """Stop sending PWM signal to prevent jitter when holding position."""
...
if SERVO_AVAILABLE:
    gpio_chip = lgpio.gpiochip_open(4)  # chip 4 for Pi 5 GPIO
    set_servo_angle(gpio_chip, pan_pin, 90)   # start centered
    time.sleep(0.5)
    set_servo_angle(gpio_chip, tilt_pin, 90)
    time.sleep(0.5)
    # Stop signals after reaching position to prevent jitter
    stop_servo(gpio_chip, pan_pin)
    stop_servo(gpio_chip, tilt_pin)
    ...
    if SERVO_AVAILABLE and (now - last_servo_update) >= servo_update_interval:
        ...
        if len(detections) > 0:
            ...
            if best_conf > float(min_thresh):
                ...
                # Move servos, hold briefly, then stop signal to prevent jitter
                set_servo_angle(gpio_chip, pan_pin, current_pan)
                set_servo_angle(gpio_chip, tilt_pin, current_tilt)
                time.sleep(0.3)  # hold signal long enough for servo to reach position
                stop_servo(gpio_chip, pan_pin)
                stop_servo(gpio_chip, tilt_pin)
...
if SERVO_AVAILABLE:
    set_servo_angle(gpio_chip, pan_pin, 90)  # Return to center
    set_servo_angle(gpio_chip, tilt_pin, 90)
    time.sleep(0.5)
    stop_servo(gpio_chip, pan_pin)
    stop_servo(gpio_chip, tilt_pin)
