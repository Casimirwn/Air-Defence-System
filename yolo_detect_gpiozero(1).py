...
position_buffer_len = 5  # number of frames to average over
pan_position_buffer = []
    ...
    if SERVO_AVAILABLE and (now - last_servo_update) >= servo_update_interval:
        ...
        if len(detections) > 0:
            ...
            if best_conf > float(min_thresh):
                ...
                # Add latest position to rolling buffers
                ...
                # Use averaged position to reduce noise-induced twitching
                smooth_bx = np.mean(pan_position_buffer)
                smooth_by = np.mean(tilt_position_buffer)

                # Error: how far off-center the object is (-1 to +1)
                # Both axes are negated to compensate for the camera being mounted 180 degrees rotated
                ...
                # Increase this value (0.0–1.0) if twitching persists
                deadzone = 0.1
                ...
                if abs(error_x) > deadzone:
                    new_pan = max(-90, min(90, current_pan + error_x * pan_speed))
                if abs(error_y) > deadzone:
                    new_tilt = max(-90, min(90, current_tilt + error_y * tilt_speed))

                # Only send signal and detach if the angle actually changed
                # This prevents the detach jolt when the deadzone blocks movement
